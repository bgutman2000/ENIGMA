function [Q_LIST_ALL, SCLALE_LIST_ALL, HITS_all, SCALE, AREA, MEAN_POS_B, MEAN_NEG_B, ROI_names] ...
    = all_shape_SL_bilat_viz(P,BETA,names, fig_finish, ns_roi_color, output_dir)
...
for i=1:length(P) 
    ...
    if(sum(isnan(BETA{i})) == 0)
        [q_list, scale_list, hits, overall_scale, area_list, mean_pos_beta_list, mean_neg_beta_list, ROI_names] ...
            = print_all_hits_SL_bilat_viz(P{i}, BETA{i}, names{i}, fig_finish, ns_roi_color, output_dir);
        Q_LIST_ALL{i} = q_list;
