function [q_list, scale_list, hits, overall_scale, area_list, mean_pos_beta_list, mean_neg_beta_list, ROI_names]...
    = print_all_hits_SL_bilat_viz(P_all, BETAS_all, model_name, fig_finish, ns_roi_color, out_dir)%1 - thickness, 2 - Jacobian
% 
% display(model_name);
% return
...
for i=1:14    
    coords{i} = get_atlas_coords(ROI_ids_ord(i));
end

area{1} = (get_atlas_area(ROI_ids_ord(1))+get_atlas_area(ROI_ids_ord(8)))/2;
for i=2:7  
    area{i} = (get_atlas_area(ROI_ids_ord(i))+get_atlas_area(ROI_ids_ord(i+7)))/2;
end
% for i=1:14
%      roi_id = ROI_ids(i);
...
for RID=1:2
    ...
    for i = 1:length(ROI_ids_ord)/2
       ...
       % q_list(i,RID) = FDR(p');    
        ...
        M = logical(ones(size(p)));
        FDRmap=LangersSearchlightFDR_BG(p',M',coords{i},6,9,3);        
...
for ind_bilat=0:1
  ...
  for RID=1:2
   ...
   %     size(FDRmap)
      ...
      %  if(RID == 1)
           loc_area2  = area{rem(i-1,7)+1};  %     (startRAD_ord(roi_id2):finishRAD_ord(roi_id2));
    %    else
     ...
     %   NS_val = (min(betas_cmap(RID,:)) + 2.0*(min(betas_cmap(RID,:))-max(betas_cmap(RID,:)))/col_bar_res);
        ...
        for i = ind_bilat*length(ROI_ids_ord)/2 + 1:(ind_bilat+1)*length(ROI_ids_ord)/2
            if(q_list(i,RID) == 0 && ns_roi_color == 1)
                Betas_fig{RID,i} = zeros(size(Betas_fig{RID,i}));% ones(size(Betas_fig{RID,i}))*NS_val;
            end
