function [p_vals, betas, d_vals, r_vals] = read_single_stat_csv_mega3(name, main_factor_name)
...
[str_betas,str_err, str_d, str_N1, str_N2, str_p_vals] = csvimport(name,'outputAsChar',true,'columns',{b_val_string,err_string, d_val_string,'"n.controls"','"n.patients"',p_val_string});%,'delimiter','","');
...
for i=1:length(betas)
    ...
    N_vals1 = str2double(str_N1{i}(2:length(str_N1{i})-1) );
    N_vals2 = str2double(str_N2{i}(2:length(str_N2{i})-1) );
    
    DF = N_vals1 + N_vals2 - DF_adj;
    t = 0; 
    if(err_vals > 0.0)
        t = betas(i)/err_vals;
    else
